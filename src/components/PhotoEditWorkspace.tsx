...
type Draft = { source: MediaFile | null; prompt: string; seed: number; mode: 'turbo' | 'quality'; resolution: FrameResolution; result: MediaFile | null; job: EditJob | null }
...
function readDraft(): Draft {
  const fresh: Draft = { source: null, prompt: '', seed: Math.floor(Math.random() * 1_000_000_000), mode: 'turbo', resolution: defaultFrameResolution, result: null, job: null }
  try {
    const saved = JSON.parse(localStorage.getItem(storageKey) ?? '{}') as Partial<Draft>
    return { ...fresh, ...saved, resolution: saved.resolution?.mode ? saved.resolution : defaultFrameResolution, mode: saved.mode === 'quality' ? 'quality' : 'turbo', source: saved.source?.path ? saved.source : null, result: saved.result?.path ? saved.result : null, job: saved.job?.id && saved.job.url ? saved.job : null }
  } catch { return fresh }
}
...
export function PhotoEditWorkspace({ url, info, connected, outputDirectory, incomingSource, onConsumeIncoming, onUseRipple, onOpenRipple }: {
  ...
  useEffect(() => {
    localStorage.setItem(storageKey, JSON.stringify({ source: source ? { ...source, preview: undefined } : null, prompt, seed, mode, resolution, result: result ? { ...result, preview: undefined } : null, job }))
  }, [source, prompt, seed, mode, resolution, result, job])
  useEffect(() => {
    let active = true
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (entry?.status?.status_str === 'error') throw new Error('FireRed failed in ComfyUI. Check its execution log and the selected model files.')
        ...
        if (entry?.status?.status_str === 'success') {
          ...
          if (missingOutputPolls > 2) throw new Error('FireRed finished without a saved image. Check the SaveImage node in ComfyUI.')
          if (active) setMessage('Collecting the edited image…')
        } else if (active) setMessage('FireRed is editing the photo in ComfyUI…')
      } catch (reason) {
        if (active) {
          const problem = reason instanceof Error ? reason.message : String(reason)
          if (problem.startsWith('FireRed finished, but')) setSaveProblem(problem)
          else setJob(null)
          setMessage(problem); setError(true)
        }
        return
      }
  ...
  const render = async () => {
    ...
    try {
      ...
      const graph = buildFireRedEditWorkflow(uploaded, prompt, seed, selected, mode, outputSize)
  ...
  const cancel = async () => {
    ...
    try { await window.minimax.cancelPrompt(job.url, job.id); setMessage('Photo edit cancelled.'); setError(false) }
  ...
  const reset = () => { setSource(null); setPrompt(''); setMode('turbo'); setResolution(defaultFrameResolution); setResult(null); setJob(null); setSaveProblem(''); setSeed(Math.floor(Math.random() * 1_000_000_000)); setMessage('Photo edit cleared.'); setError(false) }
  ...
  return <div className="photo-edit-workspace">
    ...
    <div className="photo-edit-grid">
      <div className="photo-edit-left">
        <section className="photo-edit-card"><div className="photo-edit-card-heading"><span>01</span><div><h2>Source photo</h2><p>The frame you want to change.</p></div></div><button className="photo-edit-image-picker" type="button" onClick={() => void chooseSource()} disabled={busy}>{sourceUrl ? <img src={sourceUrl} alt="Source photo for FireRed edit" onLoad={event => setSourceSize({ width: event.currentTarget.naturalWidth, height: event.currentTarget.naturalHeight })}/> : <span><ImagePlus size={31}/><strong>Choose a photo</strong><small>Or send frame 0 from Ripple</small></span>}</button>{source && <div className="photo-edit-file"><span title={source.path}>{source.name}</span><button type="button" onClick={() => void chooseSource()} disabled={busy}>Replace</button></div>}<FrameResolutionControl source={sourceSize} value={resolution} onChange={setResolution} disabled={busy}/></section>
        <section className="photo-edit-card"><div className="photo-edit-card-heading"><span>02</span><div><h2>Describe the edit</h2><p>Keep the framing and subjects you want Ripple to preserve.</p></div></div><label className="photo-edit-prompt" htmlFor="fire-red-prompt">What should change?<textarea id="fire-red-prompt" value={prompt} onChange={event => setPrompt(event.target.value)} rows={5} disabled={busy} placeholder="Change the jacket to deep red. Keep the person's face, pose, camera angle, lighting and background unchanged." /></label><small className="photo-edit-hint">FireRed uses the source photo and this instruction to create a new frame.</small></section>
      </div>

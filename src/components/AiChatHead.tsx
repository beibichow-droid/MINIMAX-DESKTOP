...
const modeCopy: Record<ChatMode, { label: string; placeholder: string; instruction: string }> = {
  prompt: { label: 'Ask', placeholder: 'Ask anything about your project…', instruction: 'Act as the Oyama AI Video Studio copilot.' },
  ...
  const send = async () => {
    ...
    try {
      ...
      if (activeMode === 'prompt') {
        const askInstruction = `${userDirectedInstruction}\n\nYou are the friendly, capable creative assistant inside Oyama AI Video Studio. Have a natural conversation and answer the user's actual question directly. You can explain, brainstorm, compare options, troubleshoot, or give production advice. Use the active workspace as helpful context, but do not force every answer into a video prompt, a fixed template, JSON, or a terse one-line response. Use short paragraphs or bullets when they improve clarity. If the user asks you to draft wording, provide it; otherwise respond like an assistant rather than a prompt generator. Be honest about uncertainty and do not claim to see details that are not present in the supplied context or images.\n\n${workspace}${recent ? `\n\nRECENT CONVERSATION:\n${recent}` : ''}\n\nUSER:\n${request}\n\nASSISTANT:`
        let reply: string
        if (images.length) {
          try { reply = await window.minimax.generateWithOllamaVision(ollamaUrl, ollamaModel, askInstruction, images, provider) }
          catch { reply = await window.minimax.generateWithOllama(ollamaUrl, ollamaModel, `${askInstruction}\n\nThe attached images could not be inspected, so rely only on the written workspace context and say so if the answer depends on unseen visual details.`, provider) }
        } else reply = await window.minimax.generateWithOllama(ollamaUrl, ollamaModel, askInstruction, provider)

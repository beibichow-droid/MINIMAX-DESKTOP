  ...
  return <div className="create-page ltx-workspace">
    ...
    <div className="workspace-grid">
      <section className="composer-panel">
        ...
        <div className="render-extras"><label><input type="checkbox" checked={state.liveEnabled} onChange={(event) => set('liveEnabled', event.target.checked)} />Live preview <small>{state.liveEnabled ? liveConnected ? 'Connected · waiting for LTX frames' : 'Connecting to ComfyUI…' : 'Off'}</small></label><label className="live-preview-mode"><span>Preview source</span><select value={state.livePreviewMode} disabled={!state.liveEnabled} onChange={(event) => set('livePreviewMode', event.target.value as WorkspaceState['livePreviewMode'])}><option value="standard">Standard decoded frame</option><option value="sampling-override" disabled={!samplingPreviewAvailable}>LTX sampling override · 24 fps</option></select></label><p className={`field-help ${state.livePreviewMode === 'sampling-override' && !samplingPreviewAvailable ? 'upscale-warning' : ''}`}>{!state.liveEnabled ? 'Intermediate frames are off; render progress and the saved video remain available.' : state.livePreviewMode === 'sampling-override' && samplingPreviewAvailable ? 'LTX2 Sampling Preview Override is wired from the diffusion model to both LTX guiders at 24 fps and uses the video VAE for sampling-time frames. The latent upscaler is intentionally not attached.' : state.livePreviewMode === 'sampling-override' ? 'LTX sampling preview requires LTX2SamplingPreviewOverride from ComfyUI-KJNodes. Install or enable it, restart ComfyUI, then refresh the Local engine.' : samplingPreviewAvailable ? 'The LTX sampling override is available. Select it to receive 24 fps sampling-time frames during the render.' : 'A decoded first frame is published; install ComfyUI-KJNodes to enable sampling-time LTX previews.'}</p></div>

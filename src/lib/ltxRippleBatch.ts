...
export function planRippleChunks(duration: number, chunkSeconds: number, overlapSeconds: number): RippleChunk[] {
  ...
  while (startFrame < totalSourceFrames) {
    ...
    // Pad a short tail to the minimum valid Ripple guide instead of leaving
    // a final segment that cannot satisfy LTX's 8n + 1 output frame rule.
    const sourceFrames = Math.min(chunkSourceFrames, Math.max(48, Math.ceil(remaining / 8) * 8))

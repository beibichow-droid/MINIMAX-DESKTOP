...
const workspaceDefaults: PersistedWorkspace = {
  ...
  advanced: false, liveEnabled: true, livePreviewMode: 'auto', previewModeVersion: 1, upscaleMode: 'off', h3ProReview: true, h3ProRefineSteps: 6, textEncoderPreference: 'fast', turbo8Profile: 'balanced', rtxModel: '', firstFrame: null,
...
function App() {
  ...
  const captureWorkspaceProject = (scope: WorkspaceProjectScope): Record<string, unknown> => {
    if (scope === 'create') return {
      ...
      seed, ref2vaSeed, seedLocked, advanced, liveEnabled, livePreviewMode, previewModeVersion: 1, upscaleMode, h3ProReview, h3ProRefineSteps, textEncoderPreference, turbo8Profile, rtxModel, firstFrame: withoutPreview(firstFrame), lastFrame: withoutPreview(lastFrame),
  ...
  return (
    <div className={`app-shell ${sidebarOpen ? '' : 'sidebar-collapsed'} ${view === 'create' ? 'video-shell-active' : `studio-shell-active studio-view-${view}`}`}>
      ...
      <main className="main-area" ref={mainAreaRef}>
        ...
        {view === 'scratchpad' && <ScratchpadWorkspace provider={llmConnection.provider} url={llmConnection.url} model={llmConnection.model} models={ollamaModels} available={ollamaModels.length > 0} onModelChange={changeCopilotModel} onSend={sendScratchpadPrompt} onNotice={(tone, text) => setNotice({ tone, text })} />}

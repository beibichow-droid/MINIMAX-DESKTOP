...
const { samplerProgressSummary, samplerTimingUpdate, nodeTimingUpdate, samplerPassForNode, nodeStageLabel } = load('src/lib/renderProgress.ts')
...
assert.equal(samplerProgressSummary(firstPass, 5500).remainingMs, 12500, 'Sampling ETA includes refinement and the elapsed part of the current step')
assert.equal(samplerProgressSummary({ ...firstPass, refinementStepCostMultiplier: 4 }, 5500).remainingMs, 21500, 'Upscale refinement estimates its larger latent grid')
...
assert.equal(samplerProgressSummary(refinePass, 6500).remainingMs, 7500, 'ETA follows measured refinement pace once that pass begins')
assert.equal(samplerProgressSummary(refinePass, 6500).currentStep, 31, 'Progress counts both sampling passes')
assert.equal(samplerProgressSummary({ ...firstPass, refinementSteps: undefined }, 5500).remainingMs, 9500, 'Single-pass ETA accounts for current-step progress')
assert.equal(samplerTimingUpdate({ currentStep: 2, lastSamplerStepAt: 1000, estimatedSamplerStepMs: 1000 }, { currentStep: 4 }, 3000).estimatedSamplerStepMs, 1000, 'A two-step progress jump measures time per step')
assert.equal(nodeTimingUpdate({ activeNodeId: '15', activeNodeStartedAt: 1000 }, '16', 4500).lastNodeDurationMs, 3500, 'Node transitions capture the completed node duration')
assert.equal(samplerPassForNode('38', 'SamplerCustomAdvanced'), 'refine', 'LTX second sampler is a refinement pass')
assert.equal(samplerPassForNode('111', 'MinimaxH3LatentUpscaler3DRefineHandoff'), 'refine', 'H3 Pro integrated upscaler is a refinement pass')
assert.equal(nodeStageLabel('VAEDecode', '16'), 'Decoding video frames', 'Registered graph node types give meaningful live stages')
...
for (const mode of ['text', 'image', 'frames', 'reference']) for (const duration of [2, 3, 5, 15]) {
  ...
  assert.ok(g['19'] && g['70'])
  for (const node of Object.values(g).filter((node) => node.class_type === 'SaveVideo')) {
    assert.equal(node.inputs['format.codec'], 'auto', 'SaveVideo requires the active format codec socket')
    assert.equal(node.inputs.codec, undefined)
  }
...
const h3WithoutPreview = buildMiniMaxWorkflow({ mode: 'text', width: 768, height: 768, prompt: 'test', duration: 2, seed: 1, steps: 20, turbo: 'off', sampler: 'res_multistep', scheduler: 'simple', filenamePrefix: 'h3-no-preview', refImageSize: 'match', livePreview: false }, models, { images: [], videos: [], audios: [] })
assert.equal(h3WithoutPreview['71'], undefined, 'Live preview off removes the H3 preview frame node')
assert.equal(h3WithoutPreview['72'], undefined, 'Live preview off removes the H3 PreviewImage output')
assert.ok(h3WithoutPreview['19'], 'Live preview off keeps the saved video output')
const h3ProWithoutPreview = buildMiniMaxWorkflow({ mode: 'text', width: 768, height: 768, prompt: 'test', duration: 2, seed: 1, steps: 20, turbo: 'off', sampler: 'res_multistep', scheduler: 'simple', filenamePrefix: 'h3-pro-no-preview', refImageSize: 'match', livePreview: false, upscale: { type: 'h3', model: 'minimax_h3_latent_upscaler_3d_fp16.safetensors', scale: 2, refineSteps: 3, refineDenoise: .35 } }, models, { images: [], videos: [], audios: [] })
assert.equal(h3ProWithoutPreview['72'], undefined, 'H3 Pro live preview off removes the preview output')
assert.ok(h3ProWithoutPreview['113'] && h3ProWithoutPreview['114'] && h3ProWithoutPreview['19'], 'H3 Pro live preview off retains the final video and audio path')
...
assert.equal(h3LatentUpscale['16'], undefined, 'H3 Pro does not decode the first pass only for a preview')
assert.equal(h3LatentUpscale['17'], undefined, 'H3 Pro does not decode unused first-pass audio')
assert.deepEqual(JSON.parse(JSON.stringify(h3LatentUpscale['71'].inputs.image)), ['113', 0], 'H3 Pro previews the final decode instead of decoding the first pass')
...
const ltxWithoutPreview = buildLtx25Workflow({ mode: 'text', preset: 'turbo', prompt: 'test', width: 768, height: 512, duration: 2, seed: 1, filenamePrefix: 'ltx-no-preview', livePreview: false }, ltxModels)
assert.equal(ltxWithoutPreview['44'], undefined, 'LTX live preview off removes the frame extraction node')
assert.equal(ltxWithoutPreview['45'], undefined, 'LTX live preview off removes the PreviewImage output')
assert.ok(ltxWithoutPreview['43'], 'LTX live preview off keeps the saved video')
for (const mode of ['text', 'image']) for (const preset of ['quality', 'turbo']) {
  ...
  assert.equal(graph['43'].inputs['format.codec'], 'auto')
...
const zimageWithoutPreview = buildZImage('test', 1024, 1024, 7, 'z.safetensors', 'qwen.safetensors', 'ae.safetensors', 40, 4, 'base', '', undefined, undefined, false)
assert.equal(zimageWithoutPreview['11'], undefined, 'Z-Image live preview off removes PreviewImage')
assert.ok(zimageWithoutPreview['10'], 'Z-Image live preview off keeps the saved image')
assert.equal(zimageBase['5'].inputs.text, ZIMAGE_DEFAULT_NEGATIVE_PROMPT)
const routedZimageBase = buildZImage('test', 1024, 1024, 7, 'z.safetensors', 'qwen.safetensors', 'ae.safetensors', 40, 4, 'base', '', undefined, { strategy: 'resident', textEncoder: { device: 'gpu:1', method: 'selector', nodeType: 'SelectCLIPDevice' } })
assert.deepEqual(JSON.parse(JSON.stringify(routedZimageBase['5'].inputs.clip)), ['802', 0], 'Both Z-Image Base prompts use the routed text encoder')
const music3 = buildMusic3Workflow({ caption: 'test', lyrics: '', duration: 30, seed: 1, tiledDecode: true, filenamePrefix: 'music-test' }, { diffusion: 'music3.safetensors', textEncoder: 'music3-clip.safetensors', vae: 'music3-vae.safetensors' })
assert.equal(music3['9'].inputs['format.quality'], 'V0', 'Music3 MP3 output requires the format quality socket')
assert.equal(music3['9'].inputs.bitrate, undefined)
